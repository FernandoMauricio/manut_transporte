...
class TransporteAdminAcompanhamentoController extends Controller
{
   ...
   public function actionImprimirDia() 
   {
       $model = new TransporteAdminAcompanhamento();
       $motoristas = Motorista::find()->where(['status' => 1])->all();

       if ($model->load(Yii::$app->request->post())) {
               return $this->redirect(['imprimir', 'motorista_id' => $model->motorista_id, 'data_confirmacao' => $model->data_confirmacao]);
           }
           return $this->renderAjax('imprimir-dia', [
               'model' => $model,
   ...
   protected function findModelImprimirDia($motorista_id, $data_confirmacao)
   {
       $queryImprimirDia = "SELECT * FROM transporte WHERE motorista_id = '".$motorista_id."' AND data_confirmacao = '".$data_confirmacao."'";
